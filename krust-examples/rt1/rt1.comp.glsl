...
float isect_sphere(const vec3 ray_origin, const vec3 ray_dir_unit, const float t_min, const vec3 sphere_centre, const float sphere_radius)
{
    ...
    if(discriminator >= 0.0f){
        ...
        // ToDo: less branchy. Duplicating roots is nasty as we are dividing (1/denominator and 2 muls?).
        // t = max(t_min, min(root1, root2));
        if(root1 > t_min){
            t = root1;
        } else {
...
void main()
{
    ...
    //Prove framebuffer origin is upper-left:
    //if(gl_GlobalInvocationID.y < 10) pixel = vec3(0,0,0.8);
    //if(screen_coord_y < 10) pixel = vec3(0.8,0,0);
